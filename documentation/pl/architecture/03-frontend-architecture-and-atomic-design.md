...
- **Organisms** (`Components/Organisms/`) — specyficzne dla funkcji, złożone z Molecules, i pierwszy poziom, któremu wolno wywoływać hooki contextu bezpośrednio (`useAlert()`, `useAccent()`) i samodzielnie robić wywołania `router.post(...)` — `IssuePageHeader`, `WorkspaceSettingsIntegrationsTab`, `NotificationsPopup` to wszystko Organisms dokładnie z tego powodu.
